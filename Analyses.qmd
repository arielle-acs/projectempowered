...
```{r internal consistency calculations including for excluded measures}
b_PAS_df <- all_data %>%
...
```{r descriptives for other measures}
DTS_stats <- all_data %>%
   drop_na(b_DTS_sum) %>%  #One parent did not complete 
  select(b_DTS_sum) %>% 
  summarize(n = n(), 
            across(everything(),  list(mean = mean,  sd = sd))) 
#Will include to characterize sample

PEDEQ_stats <- all_data %>%
   drop_na( b_PEDEQ_global) %>%  #Five parents did not complete 
  select( b_PEDEQ_global) %>% 
  summarize(n = n(), 
            across(everything(),  list(mean = mean,  sd = sd))) 
#Mean is lower than the generally observed clinical cutoff, yet these patients are hospitalized, suggesting problems with parent-report and/or measure comprehensibility 
...
PAS_stats <- all_data %>%
  ...
  summarize(n = n(), 
            across(everything(),  list(mean = mean,  sd = sd))) 
#Even though we have this data, not a good idea to interpret the PAS given incredibly poor internal consistency
```
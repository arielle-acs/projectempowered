...
filtered_baseline_data <- filtered_baseline_data %>% 
  select(-dem_childgender, -dem_txtype)
...
filtered_baseline_data <- filtered_baseline_data %>%
  mutate(across(c(pi_pfs_1:pi_pfs_7,  starts_with("dem_childage"), starts_with("dem_childgrade"),  starts_with("b_AED"), starts_with("b_PEDEQ"),  starts_with("b_PAS"),  starts_with("b_DTS"), starts_with("b_distressnow"), starts_with("pi_PAS"),  starts_with("pi_distressnow")),  
                as.numeric))
```
...
#I'm noticing problematic NAs for 4 rows of participants
#2 seem to be due to incomplete measures - will likely drop 961945 and 744436 from PAS analysis (rather than impute)
#2 seem to be due to incorrect IDs (655742 vs 908839; 392330 vs 788668)
```
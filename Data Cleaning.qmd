...
#Label completed dataframe accordingly
all_data <- filtered_baseline_data_with_emr
```
...
all_data  <- all_data  %>%
  mutate(pi_pfs_sum = pi_pfs_1 + pi_pfs_2 + pi_pfs_3 + pi_pfs_4 + pi_pfs_5 + pi_pfs_6 + pi_pfs_7, #mean total scores derived in Analysis
         b_PAS_sum =  (b_PAS_1 + b_PAS_2 + b_PAS_3 + b_PAS_4 + b_PAS_5 + b_PAS_6 + b_PAS_7) / 7,
         pi_PAS_sum = (pi_PAS_1 + pi_PAS_2 + pi_PAS_3 + pi_PAS_4 + pi_PAS_5 + pi_PAS_6 + pi_PAS_7) / 7,
         b_DTS_sum = (b_DTS_1 + b_DTS_2 + b_DTS_3 + b_DTS_4 + b_DTS_5 + b_DTS_6 + b_DTS_7 + b_DTS_8 + b_DTS_9 + b_DTS_10 + b_DTS_11 + b_DTS_12 + b_DTS_13 + b_DTS_14 + b_DTS_15 + b_DTS_16) / 16,
...
missing_PAS <- all_data %>%
  filter(if_any(c(b_PAS_1:b_PAS_7, pi_PAS_1:pi_PAS_7), is.na)) %>%
  pull(Participant_ID)

missing_DTS <- all_data %>%
  filter(if_any(c(b_DTS_1:b_DTS_16), is.na)) %>%
  pull(Participant_ID)

missing_pedeq <- all_data %>%
  filter(if_any(c(b_PEDEQ_1, b_PEDEQ_2a, b_PEDEQ_3, b_PEDEQ_4, b_PEDEQ_5, b_PEDEQ_7,
        ...
        b_PEDEQ_12, b_PEDEQ_23, b_PEDEQ_25, b_PEDEQ_26), is.na)) %>%
  pull(Participant_ID)
...
#Will need to drop 744436, 7734501461, 7739367698, 269847, and 961945 for PEDEQ analysis due to missing data

#No missing data on two investigator-developed current distress / caregiver preparedness items 
```

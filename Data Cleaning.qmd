...
filtered_baseline_data  <- filtered_baseline_data  %>%
  mutate(across(c(pi_pfs_1:pi_pfs_7,  contains("dem_childage"), contains("dem_childgrade"), contains("dem_admitlength"), contains("dem_admitbmi")),
                as.numeric))
```
...
filtered_baseline_data <- filtered_baseline_data %>%
  mutate(
    dem_education_dichotomized = case_when(
      ...
      factor(levels = c("Less Education", "More Education"))
  )
